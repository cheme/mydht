...
pub trait RandCache<K, V: Clone>: KVCache<K, V> {
  ...
  fn exact_rand(&mut self, min_no_repeat: usize, nb_tot: usize) -> Result<Vec<V>> {
    ...
    // so costy
    self.strict_fold_c(
      (&mut result, &positions, 0),
      |(result, positions, i), (_, v)| {
...
pub trait KVCache<K, V>: Sized + Cache<K, V> {
  ...
  fn update_val_c<F>(&mut self, &K, f: F) -> Result<bool>
  where
    F: FnOnce(&mut V) -> Result<()>;

  /// fold without closure over all content
  fn strict_fold_c<'a, B, F>(&'a self, init: B, f: F) -> B
  where
    F: Fn(B, (&'a K, &'a V)) -> B,
    K: 'a,
    V: 'a;
  /// very permissive fold (allowing closure)
  fn fold_c<'a, B, F>(&'a self, init: B, f: F) -> B
  where
    F: FnMut(B, (&'a K, &'a V)) -> B,
    K: 'a,
    V: 'a;
  ...
  fn map_inplace_c<'a, F>(&'a self, mut f: F) -> Result<()>
  where
    F: FnMut((&'a K, &'a V)) -> Result<()>,
    K: 'a,
    V: 'a,
  {
  ...
  fn len_where_c<F>(&self, f: &F) -> usize
  where
    F: Fn(&V) -> bool,
  {
    self.fold_c(0, |nb, (_, v)| if f(v) { nb + 1 } else { nb })
  }
  ...
  fn next_random_values<'a, F>(&'a mut self, queried_nb: usize, f: Option<&F>) -> Vec<&'a V>
  where
    K: 'a,
    F: Fn(&V) -> bool,
  {
...
/// collection to count
///
fn inner_cache_bv_rand<'a, K, V, C: KVCache<K, V>, F: Fn(&V) -> bool>(
  c: &'a C,
  buf: &[u8],
  xess: usize,
  nb: usize,
  ...
  K: 'a,
{
  ...
  let fsize = match filtfn {
    Some(f) => {
      c.strict_fold_c((xess, 0), |(i, mut tot), (_, v)| {
        if f(v) {
          if filter[i] {
            tot = tot + 1;
          }
...
impl<K, V> KVCache<K, V> for NoCache<K, V> {
  fn update_val_c<F>(&mut self, _: &K, _: F) -> Result<bool>
  where
    F: FnOnce(&mut V) -> Result<()>,
  {
    Ok(false)
  }
  fn strict_fold_c<'a, B, F>(&'a self, init: B, _: F) -> B
  where
    F: Fn(B, (&'a K, &'a V)) -> B,
    K: 'a,
    V: 'a,
  ...
  fn fold_c<'a, B, F>(&'a self, init: B, _: F) -> B
  where
    F: FnMut(B, (&'a K, &'a V)) -> B,
    K: 'a,
    V: 'a,
...
impl<K: Hash + Eq, V> KVCache<K, V> for HashMap<K, V> {
  ...
  fn update_val_c<F>(&mut self, k: &K, f: F) -> Result<bool>
  where
    F: FnOnce(&mut V) -> Result<()>,
  {
  ...
  fn fold_c<'a, B, F>(&'a self, init: B, mut f: F) -> B
  where
    F: FnMut(B, (&'a K, &'a V)) -> B,
    K: 'a,
    V: 'a,
