...
#[cfg(feature = "mio-transport")]
use self::mio::{
  Registration, SetReadiness, Evented, Poll as MioPoll, Token as MioToken, Ready as MioReady,
  PollOpt,
};
use transport::{
  Registerable,
...
impl<C> SpawnSend<C> for NoSend {
  ...
  fn send(&mut self, _: C) -> Result<()> {
    ...
    Err(Error(
      "Spawner does not support send".to_string(),
      ErrorKind::Bug,
      None,
    ))
  }
  ...
  Spawner<S, D, R> for Blocker
{
  ...
  fn spawn(
    ...
    spawn_loop!(
      ...
      err,
      Err(Error(
        "Blocking spawn service return would return when should loop".to_string(),
        ErrorKind::Bug,
        None
      ))
    );
...
impl<S, D, R> SpawnHandle<S, D, R> for CoroutHandle<S, D, R> {
  ...
  fn unwrap_state(self) -> Result<(S, D, R, Result<()>)> {
    match self.0.replace(None) {
      Some(s) => Ok(s),
      None => Err(Error(
        "Read an unfinished corouthandle".to_string(),
        ErrorKind::Bug,
        None,
      )),
    }
    ...
    let co_handle = CoroutineC::spawn(move |corout, _| {
      move || -> Result<()> {
        ...
        spawn_loop!(
          ...
          err,
          Err(Error(
            "Coroutine spawn service return would return when should loop".to_string(),
            ErrorKind::Bug,
            None
          ))
        );
...
impl<S, D, R> SpawnHandle<S, D, R> for ThreadHandleBlock<S, D, R> {
  ...
  fn unwrap_state(self) -> Result<(S, D, R, Result<()>)> {
    ...
    if mlock.is_some() {
      ...
      Err(Error(
        "unwrap state on unfinished thread".to_string(),
        ErrorKind::Bug,
        None,
      ))
    }
  ...
  fn spawn(
    ...
    let join_handle = thread::Builder::new().spawn(move || {
      ...
      spawn_loop!(
        ...
        err,
        Err(Error(
          "Thread block spawn service return would return when should loop".to_string(),
          ErrorKind::Bug,
          None
        ))
      );
  ...
  fn spawn(
    ...
    let join_handle = thread::Builder::new().spawn(move || {
      ...
      spawn_loop!(
        ...
        err,
        Err(Error(
          "Thread park spawn service return would return when should loop".to_string(),
          ErrorKind::Bug,
          None
        ))
      );
    ...
    let future = self.0.spawn_fn(move || {
      match move || -> Result<(S, D, R, Result<()>)> {
        ...
        spawn_loop!(
          ...
          err,
          Err(Error(
            "CpuPool spawn service return would return when should loop".to_string(),
            ErrorKind::Bug,
            None
          ))
        );
