...
/// trust hashmap).
pub struct SimpleCache<V: KeyVal, C: KVCache<<V as KeyVal>::Key, V>>
where
  V::Key: Hash,
{
  cache: C,
...
impl<K1: Key + Hash, K2: Key + Hash, V: KeyVal<Key = (K1, K2)>, C: KVCache<(K1, K2), V>>
  KVStoreRel<K1, K2, V> for SimpleCache<V, C>
{
  fn get_vals_from_left(&self, k1: &K1) -> Vec<V> {
    self
      .cache
      .strict_fold_c(Vec::new(), |mut r, (ref k, ref v)| {
        if k.0 == *k1 {
          r.push((*v).clone())
        };
        r
  ...
  T::Key: Hash,
{
  ...
  fn get_next_vals(&mut self, nb: usize) -> Option<Vec<T>> {
    ...
    let mut r: Vec<T> = self
      .cache
      .next_random_values(nb, t_none)
      .into_iter()
      .map(|v| v.clone())
      .collect();
    if r.len() < nb && r.len() < clen {
      ...
      if pos < clen - 1 {
        self
          .cache
          .fold_c((0, &mut r, adjust), |(i, res, mut rem), (_k, val)| {
  ...
  T::Key: Hash,
{
  ...
  fn commit_store_new(&mut self) -> MDHTResult<()> {
    match self.persi {
      Some(ref confpath) => {
        let mut conffile = OpenOptions::new()
          .read(true)
          .write(true)
          .open(confpath)
          .unwrap();
        ...
        let vser: Vec<&T> = self.cache.strict_fold_c(Vec::with_capacity(l), |mut v, p| {
          v.push(p.1);
          v
        });
...
impl<V: KeyVal> SimpleCache<V, HashMap<<V as KeyVal>::Key, V>>
where
  V::Key: Hash,
{
    ...
    let map = match &mut inifile {
      &mut Some(ref mut p) => {
        if !new {
          ...
          let vser: Vec<V> = json::from_str(&jcont[..])
            .unwrap_or_else(|e| panic!("Invalid config {:?}\n quiting", e));
